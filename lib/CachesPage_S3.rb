...
module ActionController #:nodoc:
  module Caching
    ...
    # likely candidates.
    #
    # Specifying which actions to cache is done through the <tt>caches_page_to_s3</tt> class method:
    #
    #   class WeblogController < ActionController::Base
    #     caches_page_to_s3 :show, :new
    #   end
    #
    ...
    module PagesS3
      ...
      module ClassMethods
        ...
        def cache_page_s3(content, path)
          return unless perform_caching
          path = page_cache_path(path)
        ...
        #   caches_page_to_s3 :index
        #
        #   # cache the index action except for JSON requests
        #   caches_page_to_s3 :index, :if => Proc.new { |c| !c.request.format.json? }
        def caches_page_to_s3(*actions)
          return unless perform_caching
          options = actions.extract_options!
          after_filter({:only => actions}.merge(options)) { |c| c.cache_page_to_s3 }
        end
      ...
      def cache_page_to_s3(content = nil, options = nil)
        ...
        PagesS3::copy_cached_page_to_s3(bucket, name, cache_path) unless
